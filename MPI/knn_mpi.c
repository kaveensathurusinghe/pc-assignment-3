...
int main(int argc, char *argv[]) {
    ...
    total_test_points = test_data->num_points;
    ...
    if (rank == 0) {
        ...
        num_classes = (max_train_label > max_test_label ? max_train_label : max_test_label) + 1;
    ...
    int base_count = total_test_points / size;
    int remainder = total_test_points % size;
    
    int start_idx, end_idx, local_count;
    if (rank < remainder) {
        local_count = base_count + 1;
        start_idx = rank * local_count;
    } else {
        local_count = base_count;
        start_idx = rank * base_count + remainder;
    }
    end_idx = start_idx + local_count;
    
    MPI_Barrier(MPI_COMM_WORLD);
    start_time = MPI_Wtime();
    
    int *local_predictions = (int*)malloc(local_count * sizeof(int));
    int *local_true_labels = (int*)malloc(local_count * sizeof(int));
    ...
    for (int i = start_idx; i < end_idx; i++) {
        ...
        local_predictions[local_idx] = predicted_label;
        local_true_labels[local_idx] = true_label;
    ...
    for (int i = 0; i < local_count; i++) {
        if (local_predictions[i] == local_true_labels[i]) {
            local_correct++;
        }
    }
    
    int *all_predictions = NULL;
    int *all_true_labels = NULL;
    int *recv_counts = NULL;
    int *displs = NULL;
    
    if (rank == 0) {
        all_predictions = (int*)malloc(total_test_points * sizeof(int));
        all_true_labels = (int*)malloc(total_test_points * sizeof(int));
    ...
    MPI_Gatherv(local_predictions, local_count, MPI_INT,
                all_predictions, recv_counts, displs, MPI_INT,
                0, MPI_COMM_WORLD);
    
    MPI_Gatherv(local_true_labels, local_count, MPI_INT,
                all_true_labels, recv_counts, displs, MPI_INT,
                0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        double accuracy = (double)total_correct / total_test_points * 100.0;
        double execution_time = (end_time - start_time) * 1000.0; 
        
        printf("\nResults:\n");
        printf("  Correct predictions: %d/%d\n", total_correct, total_test_points);
        printf("  Accuracy: %.2f%%\n", accuracy);
        ...
        free(all_true_labels);
        free(recv_counts);
        free(displs);
    }
    
    free(local_predictions);
    free(local_true_labels);
    free_dataset(train_data);
    free_dataset(test_data);
